...
void delete_one_from_each_of_element_arr(element* arr, int size);
...
int main()
{
	int r = 0;
	r = my_create(4, 1);
	r = my_create_object("o1");
	r = my_create_object("o2");
	r = my_create_object("o3");
	r = my_create_object("o4");
	r = my_link("o2", "o1");
	r = my_link("o2", "o2");

	r = my_destroy_object("o1");
	...
	for (int i = 0; i < size; i++)
	{
		if (i == index)
		{
			...
			if (ARR[i].numb_of_links != 0)
				delete_one_from_each_of_element_arr(ARR[i].links, 16);
			continue;
		}
